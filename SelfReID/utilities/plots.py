...
def scatter(x, labels=None, title=None, subtitle=None, filename=None):
    ...
    if labels is not None:
        ...
        label_colours = np.array([palette[int(np.where(unique_classes == label)[0])] for label in labels])
...
def create_gif(source:str, dest_folder:str, name:str, format:str = "png", delay:int = 5):
    ...
    gif_name = name + ".gif"
    images[0].save(os.path.join(dest_folder, gif_name), save_all=True, append_images=images[1:], optimize=False, duration=600)
